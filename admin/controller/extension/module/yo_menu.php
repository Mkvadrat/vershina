...
class ControllerExtensionModuleYoMenu extends Controller{
    ...
    public function index(){
        ...
        if(isset(
            $this->error['name'])){$data['error_name']=$this->error['name'];
        }else{
            $data['error_name']='';
        }
        ...
        if(!isset($this->request->get['module_id'])){
            $data['breadcrumbs'][]=array(
                ...
                'href'=>$this->url->link('extension/module/yo_menu',
                'token='.$this->session->data['token'],true)
            );
        }else{
            $data['breadcrumbs'][]=array(
        ...
        if(isset($this->request->post['yo_module'])){
            ...
            $data['yo_module']['allcats']=1;$data['yo_module']['cat_invert']=1;
        }
    ...
    public function install(){
        $image=$this->db->query("DESC `".DB_PREFIX."category` `yomenu_image`");if(!$image->num_rows){$this->db->query("ALTER TABLE `".DB_PREFIX."category` ADD `yomenu_image` varchar(255) NULL AFTER image");}$icon=$this->db->query("DESC `".DB_PREFIX."category` `yomenu_icon`");if(!$icon->num_rows){$this->db->query("ALTER TABLE `".DB_PREFIX."category` ADD `yomenu_icon` varchar(255) NULL AFTER image");}$content=$this->db->query("DESC `".DB_PREFIX."category` `yomenu_content`");if(!$content->num_rows){$this->db->query("ALTER TABLE `".DB_PREFIX."category` ADD `yomenu_content` TEXT NOT NULL");}$brand_image=$this->db->query("DESC `".DB_PREFIX."manufacturer` `yomenu_image`");if(!$brand_image->num_rows){$this->db->query("ALTER TABLE `".DB_PREFIX."manufacturer` ADD `yomenu_image` varchar(255) NULL AFTER image");}$brand_icon=$this->db->query("DESC `".DB_PREFIX."manufacturer` `yomenu_icon`");if(!$brand_icon->num_rows){$this->db->query("ALTER TABLE `".DB_PREFIX."manufacturer` ADD `yomenu_icon` varchar(255) NULL AFTER image");}$brand_content=$this->db->query("DESC `".DB_PREFIX."manufacturer` `yomenu_content`");if(!$brand_content->num_rows){$this->db->query("ALTER TABLE `".DB_PREFIX."manufacturer` ADD `yomenu_content` TEXT NOT NULL");}
    }
}
